...
const MainSection2: React.FC = () => {
  const stores = [
    { name: '부산 기장점', img: shop1 },
    { name: '서울 강남점', img: shop2 },
    { name: '제주 애월점', img: shop3 },
    { name: '대구 수성점', img: shop4 },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stores.map((store, index) => (
            ...
            >
              ...
              {/* Bottom Shadow Overlay & Text */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-90 transition-opacity group-hover:from-brand-yellow/90" />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 transform transition-transform duration-500 group-hover:-translate-y-2">
                ...
                <h3 className="text-2xl font-black text-white group-hover:text-brand-dark transition-colors">
                  {store.name}
                </h3>
              </div>

              {/* Hover Detail Button (Optional) */}
