...
const Navbar: React.FC = () => {
    ...
    const textColorClass = isNavbarActive ? 'text-brand-dark' : 'text-white/80';
    ...
    return (
        ...
        >
            <div className={twMerge(["transition-all duration-300", isNavbarActive ? "py-4" : "py-6"])}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-12 items-center">
                                ...
                                {navLinks.map((link) => (
                                    <div key={link.name} className="text-center relative group">
                                        <Link to={link.path} className={twMerge([
                                            "inline-block py-4 text-lg font-black transition-all duration-200 origin-center",
                                            "hover:text-brand-yellow hover:scale-125",
                                            textColorClass
                                        ])}>{link.name}</Link>
                                    </div>
                                ))}
                        ...
                        <div className={twMerge(["hidden md:flex col-span-3 justify-end items-center space-x-4 border-l pl-6 border-brand-gray h-8"])}>
                            {user ? (
                                <div className="flex items-center space-x-5">
                                    {isAdmin && (
                                        <Link to="/admin" className={twMerge(["flex flex-col items-center hover:text-brand-yellow transition-colors", textColorClass])}>
                                            <Settings size={20} /><span className="text-[10px] font-black mt-0.5">ADMIN</span>
                                        </Link>
                                    )}
                                    <Link to="/mypage" className={twMerge(["flex flex-col items-center hover:text-brand-yellow transition-colors", textColorClass])}>
                                        <User size={20} /><span className="text-[10px] font-black mt-0.5">MY</span>
                                    </Link>
                                    <Link to="/cart" className={twMerge(["flex flex-col items-center hover:text-brand-yellow transition-colors relative", textColorClass])}>
                                        <ShoppingBag size={20} /><span className="text-[10px] font-black mt-0.5">CART</span>
                                    </Link>
                                    <button onClick={logout} className={twMerge(["flex flex-col items-center hover:text-red-500 transition-colors", textColorClass])}>
                                        <LogOut size={20} /><span className="text-[10px] font-black mt-0.5">OUT</span>
                                    </button>
                                </div>
                            ) : (
                                <>
            ...
            <div className={twMerge([
                "hidden md:block absolute left-0 w-full bg-white border-t border-gray-100 shadow-lg transition-all duration-300 overflow-hidden",
                isHovered ? "max-h-96 opacity-100 visible" : "max-h-0 opacity-0 invisible"
            ])}>
