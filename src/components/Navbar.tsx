...
const Navbar: React.FC = () => {
   ...
   const [isLargeScreen, setIsLargeScreen] = useState(true); // [추가] 화면 크기 상태
   ...
   useEffect(() => {
      ...
      // [추가] 화면 크기 감지 핸들러
      const handleResize = () => setIsLargeScreen(window.innerWidth >= 1281);
   ...
   const isNavbarActive =
      (!isHome && !isMenu) || isScrolled || isHovered || isMenuOpen;
      ...
      <nav
                  ...
                  <div className="hidden md:flex flex-1 justify-center relative">
                     <div
                        ...
                        {navLinks.map(link => (
                           <div
                              key={link.name}
                              className="w-32 text-center relative group py-2">
                              <Link
                                 to={link.path}
                                 className={twMerge([
                     ...
                     {user ? (
                        <div className="flex items-center space-x-10">
                           {isAdmin && (
                              <Link
                                 to="/admin"
                                 className={twMerge([
                                    "flex items-center space-x-1.5 hover:text-brand-yellow transition-colors whitespace-nowrap",
                                    textColorClass,
                                 ])}>
                                 <Settings size={20} />
                           ...
                           <Link
                              ...
                              <User size={20} />
                              <span className="text-[12px] font-black">MY</span>
                           </Link>
                           <Link
                              to="/cart"
                              className={twMerge([
                                 "flex items-center space-x-1.5 hover:text-brand-yellow transition-colors relative whitespace-nowrap",
                                 textColorClass,
                              ])}>
                              <ShoppingBag size={20} />
                              <span className="text-[12px] font-black">
                           ...
                           <button
                              onClick={logout}
                              className={twMerge([
                                 "flex items-center space-x-1.5 hover:text-red-500 transition-colors whitespace-nowrap",
                                 textColorClass,
                              ])}>
                              <LogOut size={20} />
                              <span className="text-[12px] font-black">
                        ...
                        <div className="flex items-center space-x-4 whitespace-nowrap">
                           <Link
                              to="/login"
                              className={twMerge([
                                 "relative flex items-center space-x-1 text-sm font-bold hover:text-brand-yellow transition-colors",
                                 textColorClass,
                              ])}>
                              <LogIn size={18} />
                              <span>로그인</span>
                           ...
                           <Link
                              to="/signup"
                              className="flex items-center space-x-1 text-sm font-bold bg-brand-yellow text-brand-dark px-3 py-1.5 rounded-full hover:opacity-80 transition-colors">
                              <User size={18} />
                              <span>회원가입</span>
                           </Link>
                        </div>
         ...
         <div
            className={twMerge([
               ...
               isHovered
                  ? "max-h-96 opacity-100 visible delay-300"
