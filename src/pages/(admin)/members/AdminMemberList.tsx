...
function AdminMemberList() {
   ...
   const memberStats = useMemo(() => {
      ...
      orders.forEach((order: any) => {
         ...
         const isPaid =
            ...
            st.includes("SUCCESS") ||
            st.includes("PAID") ||
            st.includes("PREPARING") ||
            st.includes("SHIPPING") ||
   ...
   const updateGradeMutation = useMutation({
      mutationFn: ({ id, grade }: { id: number; grade: string }) =>
         adminMemberApi.updateMember(id, { grade }),
      onSuccess: () => {
   ...
   const toggleStatusMutation = useMutation({
      mutationFn: (member: any) =>
         adminMemberApi.updateMember(member.id, {
      ...
      onSuccess: () => {
         ...
         useAlertStore
            .getState()
            .showAlert("상태가 변경되었습니다.", "성공", "success");
      },
   });
   ...
   const pointMutation = useMutation({
      ...
      onSuccess: () => {
         useAlertStore
            .getState()
            .showAlert("포인트가 지급되었습니다.", "성공", "success");
      ...
      onError: (err: any) =>
         useAlertStore
            .getState()
            .showAlert(
               `지급 실패: ${err.response?.data?.message || err.message}`,
               "실패",
               "error",
            ),
   });
   ...
   useEffect(() => {
      if (isOrdersLoading || isMembersLoading || sortedMembers.length === 0)
         return;
      ...
      if (membersToSync.length > 0) {
         ...
         updateGradeMutation.mutate({
            id: target.id,
            grade: getDynamicGrade(getMemberTotal(target)),
         });
      } else {
   ...
   const SortIcon = ({ field }: { field: string }) => {
      ...
      return sortOrder === "asc" ? (
         <MdArrowUpward size={14} className="ml-1" />
      ) : (
         <MdArrowDownward size={14} className="ml-1" />
      );
   };
         ...
         <div className="flex justify-between items-center">
               ...
               <div className="flex items-center gap-2 mt-1">
                  ...
                  {isAutoSyncing && (
                     <div className="flex items-center gap-1.5 px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black animate-pulse">
                        <MdSync className="animate-spin" size={12} /> 등급 자동
                        동기화 중...
                     </div>
                  )}
               </div>
            ...
            <div className="flex gap-3">
               <button
                  ...
                  className="flex items-center gap-2 bg-brand-yellow text-brand-dark px-6 py-3 rounded-xl text-sm font-black hover:bg-yellow-400 transition-all shadow-sm">
                  <MdCardGiftcard size={20} /> 전체 포인트 지급
               </button>
               <Link
                  to="/admin/members/new"
                  className="flex items-center gap-2 bg-[#222222] text-white px-6 py-3 rounded-xl text-sm font-black hover:bg-black transition-all shadow-lg">
                  <MdAdd size={20} /> 회원 등록
               </Link>
            </div>
         </div>
            ...
            <div className="overflow-x-auto">
               <table className="w-full text-center">
                  <thead className="bg-gray-50 text-sm font-black text-gray-600 uppercase tracking-widest border-b border-gray-50">
                     <tr>
                        <th
                           className="px-6 py-4 cursor-pointer hover:text-[#222222] transition-colors"
                     ...
                     {isMembersLoading ? (
                        ...
                        sortedMembers.map((member: any) => (
                           <tr
                              ...
                              <td className="px-6 py-4 text-sm font-bold font-mono text-[#222222]">
                                 #{member.id}
                              </td>
                              <td className="px-6 py-4 text-sm font-bold text-[#222222]">
                                 {member.name}
                              </td>
                              <td className="px-6 py-4 text-sm font-bold text-[#222222]">
                                 {member.email}
                              ...
                              <td className="px-6 py-4">
                                 <div className="flex justify-center">
                                    <span
                                       className={`px-2 py-1 rounded text-sm font-bold w-fit transition-colors bg-gray-100 text-[#222222]`}>
                                       {member.grade}
                                    </span>
                                 </div>
                              </td>
                              ...
                              <td className="px-6 py-4 text-center">
                                 <div className="flex items-center justify-center gap-2">
                                    <button
                                       ...
                                       className="p-2 rounded-lg bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition-all border border-yellow-200 shadow-sm"
                                       title="포인트 지급">
                                       <MdCardGiftcard size={18} />
                                    </button>
                                    <Link
                                       to={`/admin/members/${member.id}`}
                                       className="p-2 rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-[#222222] transition-all border border-gray-200 shadow-sm"
                                       title="수정">
                                       <MdEdit size={18} />
                                    </Link>
                                    <button
            ...
            {membersResponse &&
               ...
               membersResponse.pagination.totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-8 py-4 border-t border-gray-50">
                     ...
                     {Array.from(
                        { length: membersResponse.pagination.totalPages },
                        (_, i) => i + 1,
                     ).map(p => (
                        <button
                           key={p}
                     ...
                     <button
                        ...
                        disabled={
                           page === membersResponse.pagination.totalPages
                        }
                        className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-30 disabled:hover:bg-transparent transition-all">
            ...
            {isPointModalOpen && (
               <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                  <motion.div
                     ...
                     className="bg-white w-full max-w-md rounded-[30px] overflow-hidden shadow-2xl p-8">
                     <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-black text-brand-dark flex items-center gap-2">
                           <MdCardGiftcard className="text-brand-yellow" />{" "}
                           {targetMember
                              ? `${targetMember.name}님에게 지급`
                              : "전체 회원 지급"}
                        </h3>
                        <button
                     ...
                     <div className="space-y-4">
                        <div>
                           ...
                           <input
                              ...
                              className="w-full px-4 py-3 bg-gray-50 rounded-xl border-none focus:ring-2 focus:ring-brand-yellow font-bold"
                           />
                        </div>
                        <div>
                           <label className="text-xs font-bold text-gray-500 mb-1 block">
                        ...
                        <button
                           ...
                           className="w-full py-4 bg-brand-dark text-white rounded-xl font-black text-lg hover:bg-black transition-all shadow-lg mt-4 disabled:opacity-50">
