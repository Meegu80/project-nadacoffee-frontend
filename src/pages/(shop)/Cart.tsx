...
function Cart() {
  ...
  // 2. 서버 응답에서 실제 배열 데이터 안전하게 추출
  const serverCart = useMemo(() => {
    if (!serverResponse) return [];
    if (Array.isArray(serverResponse)) return serverCart;
    return (serverResponse as any).data || [];
  }, [serverResponse]);

  // 3. 화면에 표시할 아이템 목록 계산 (스토어에 의존하지 않고 서버 데이터 직접 사용)
  const displayItems = useMemo(() => {
    ...
    return serverCart.map(item => {
      ...
      return {
        ...
        stockQty: item.option?.stockQty || 0,
  ...
  // 4. 서버 데이터가 변경될 때만 스토어 동기화 (무한 루프 방지)
  useEffect(() => {
            ...
            <div className="bg-white rounded-[40px] shadow-xl border border-gray-100 overflow-hidden">
              <div className="bg-gray-50/50 px-8 py-6 border-b border-gray-100 flex justify-between items-center">
                <div className="flex items-center gap-6">
                  ...
                  <button onClick={() => clearCartMutation.mutate()} className="flex items-center gap-2 text-xs font-black text-red-400 hover:text-red-600 transition-colors border-l border-gray-200 pl-6"><MdDeleteSweep size={20} /> 장바구니 비우기</button>
                </div>
                  ...
                  <div key={item.id} className="grid grid-cols-1 lg:grid-cols-12 items-center gap-4 px-8 py-10 transition-all group relative">
                    ...
                    <div className="col-span-3 flex items-center gap-6">
                      ...
                      <div className="min-w-0">
                        ...
                        {item.optionName && <span className={twMerge(["px-3 py-1 rounded-lg text-[10px] font-black shadow-sm text-white", item.optionName === 'ICE' ? 'bg-blue-500' : 'bg-red-500'])}>{item.optionName}</span>}
                      </div>
                    </div>
                    <div className="col-span-1 text-center"><span className="text-xs font-black px-3 py-1.5 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-400">{item.stockQty}개</span></div>
            ...
            <div className="bg-white rounded-[50px] shadow-2xl p-16 border border-gray-100 relative overflow-hidden">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center relative z-10">
                ...
                <div className="flex flex-col gap-8">
                  ...
                  <button onClick={handleCheckout} className="w-full py-8 bg-brand-yellow text-brand-dark rounded-[30px] font-black text-3xl hover:bg-black hover:text-white transition-all shadow-[0_25px_50px_rgba(255,212,0,0.3)] active:scale-95">주문하기</button>
                </div>
              </div>
