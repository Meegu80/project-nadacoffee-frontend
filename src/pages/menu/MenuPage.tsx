...
import { Home, ChevronDown, ChevronRight, Info, Star, AlertCircle } from 'lucide-react';
...
const MenuPage: React.FC = () => {
  ...
  const { data: allProducts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let allData: any[] = [];
      const firstRes = await getProducts({ isDisplay: 'true', limit: 100, page: 1 });
      allData = [...firstRes.data];
      const totalPages = firstRes.pagination.totalPages;
      if (totalPages > 1) {
        for (let p = 2; p <= totalPages; p++) {
          const res = await getProducts({ isDisplay: 'true', limit: 100, page: p });
    ...
    <div className="bg-white min-h-screen">
      <section className="relative w-full h-auto z-[100]">
        <div className="w-full aspect-[21/6] md:aspect-[25/4.5] min-h-[200px] relative">
          <div className="absolute inset-0 overflow-hidden"><img src={heroBanner} alt="Banner" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/10" /></div>
            ...
            <motion.div layout className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-4 md:gap-x-6 gap-y-10 md:gap-y-14">
              <AnimatePresence mode='popLayout'>
                {currentItems.map((product) => {
                  ...
                  return (
                    <motion.div key={product.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`group flex flex-col h-full ${isSoldOut ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={() => !isSoldOut && navigate(`/products/${product.id}`)}>
                      <div className="relative aspect-[3/4] overflow-hidden rounded-[20px] bg-[#F9F9F9] mb-3 shadow-md border border-[#F0F0F0]">
                        ...
                        {isLowStock && !isSoldOut && (
                          <div className="absolute top-4 right-4 z-10 animate-bounce">
                            <Badge variant="orange" size="xs" className="gap-1 shadow-lg"><AlertCircle size={10} /> 품절임박</Badge>
                          </div>
                        )}
                        ...
                        {isSoldOut && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <Badge variant="dark" outline className="text-white border-white text-2xl px-6 py-3 rotate-[-10deg]">SOLD OUT</Badge>
                          </div>
                        )}
                        ...
                        {!isSoldOut && (
                          <div className="absolute inset-0 bg-brand-yellow/90 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col p-6 backdrop-blur-[2px]">
                            <div className="flex items-center justify-center gap-2 mb-6 border-b border-brand-dark/10 pb-3"><Info size={14} className="text-brand-dark" /><span className="text-[10px] font-black text-brand-dark uppercase tracking-[0.2em]">Nutrition Info</span></div>
                            <div className="text-brand-dark"><p className="text-sm font-black mb-6 text-center leading-tight px-2">{product.name}</p><ul className="text-[10px] md:text-[11px] font-bold space-y-1.5 ml-4 md:ml-8 opacity-90"><li>⚬ 용량 : 591.00</li><li>⚬ 열량(kcal) : 256.10</li><li>⚬ 나트륨(mg) : 15.80</li><li>⚬ 탄수화물(g) : 68.50</li><li>⚬ 당류(g) : 59.40</li><li>⚬ 지방(g) : 0.40</li><li>⚬ 포화지방(g) : 0.00</li><li>⚬ 단백질(g) : 1.30</li></ul></div>
